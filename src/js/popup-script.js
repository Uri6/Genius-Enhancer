        ...
        switch (pageType) {
            case "song":
                ...
                $("#apple-music-pop-up").click(() => {
                    const appleMusicCheckbox =
                        document.getElementById("apple-music-pop-up");
                    chrome.storage.local.set({
                        appleMusicPopUp: appleMusicCheckbox.checked,
                    });
                    chrome.runtime.sendMessage({
                        song_appleMusicPopUp: [appleMusicCheckbox.checked],
                    });
                });
                ...
                $("#spotify-pop-up").click(() => {
                    const spotifyCheckbox =
                        document.getElementById("spotify-pop-up");
                    chrome.storage.local.set({
                        spotifyPopUp: spotifyCheckbox.checked,
                    });
                    chrome.runtime.sendMessage({
                        song_spotifyPopUp: [spotifyCheckbox.checked],
                    });
                });

                chrome.storage.local.get(["ModernTextEditor"], (res) => {
                    $("#modern-text-editor").prop(
                        "checked",
                        res.ModernTextEditor
                    );
                });

                $("#modern-text-editor").click(() => {
                    const ModernTextEditorCheckbox =
                        document.getElementById("modern-text-editor");
                    chrome.storage.local.set({
                        ModernTextEditor: ModernTextEditorCheckbox.checked,
                    });
                    chrome.runtime.sendMessage({
                        song_ModernTextEditor: [
                            ModernTextEditorCheckbox.checked,
                        ],
                    });
                });
                ...
                $("#old-song-page").click(() => {
                    const OldSongPageCheckbox =
                        document.getElementById("old-song-page");
                    chrome.storage.local.set({
                        OldSongPage: OldSongPageCheckbox.checked,
                    });
                });
                break;
            case "album":
                ...
                const handleCheckbox = (checkboxName, messageArray) => {
                    ...
                    $(`#${checkboxName}`).click(() => {
                        chrome.storage.local.set({
                            [checkboxName]: $(`#${checkboxName}`).prop(
                                "checked"
                            ),
                        });
                        const messageText = $(`#${checkboxName}`).prop(
                            "checked"
                        )
                            ? "album_missingInfo"
                            : "album_missingInfo_remove";
                        chrome.runtime.sendMessage({
