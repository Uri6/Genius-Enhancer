    ...
    if (appleMusicIframe.nodeName == "IFRAME") {
      ...
      const contentPoll = setInterval(() => {
        if (appleMusicIframe.contentDocument != null) {
          ...
          if (show) {
            appleMusicIframe.style.display = "";
          }
          else {
...
export function spotifyPopUp(show) {
  var spotifyElem = document.getElementById('gb-spotify-player');
  if (show) {
    spotifyElem.style.display = "";
  }
  else {
    spotifyElem.style.display = "none";
  }
}
...
export function song_modernTextEditor() {
  ...
  const observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      if (mutation.type == "attributes" && mutation.attributeName == "class" && !$(".ql-toolbar-container").length) {
        if (!mutation.target.classList.contains("ng-hide") && mutation.target.classList.contains("wysiwyg_input") && mutation.target.classList.contains("wysiwyg_input--full_width")) {
          chrome.runtime.sendMessage({ "replaceTextarea": ["wysiwyg_input wysiwyg_input--full_width"] });
          flag = true;
  ...
  if ($(".wysiwyg_input.wysiwyg_input--full_width").length) {
    observer.observe(document.getElementsByClassName("wysiwyg_input wysiwyg_input--full_width")[0], {
      attributes: true,
      childList: false,
      characterData: false
    });
  ...
  $(document).on('DOMNodeInserted', function (e) {
    if ((e.target.className == "ExpandingTextarea__Textarea-sc-4cgivl-0 kYxCOo" && e.target.parentNode.childNodes.length < 3)) {
      chrome.runtime.sendMessage({ "replaceTextarea": ["ExpandingTextarea__Textarea-sc-4cgivl-0 kYxCOo"] });

      $(document).on('DOMNodeRemoved', () => {
        setTimeout(function () {
          if (document.querySelectorAll(".LyricsEditdesktop__Controls-sc-19lxrhp-5.bwjuqY button").length < 3) {
            chrome.runtime.sendMessage({ "removeQuill": true });
          }
        }, 100);
      });
