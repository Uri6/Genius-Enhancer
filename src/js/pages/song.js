...
export async function handleSongPage(tabId) {
	...
	await chrome.scripting.executeScript({
		...
		func: async () => {
			...
			const lyricsEditorContainer = document.querySelector(".LyricsEditdesktop__Editor-sc-19lxrhp-1");
			...
			const toolbarButtons = toolbarButtonNames.map(name => createButton(name, "header-option", () =>
            addTextToTextArea(name)));

			const headerOptionButtonNames = ["Intro", "Verse", "Chorus", "Instrument", "Outro"];
			const headerOptionButtons = headerOptionButtonNames.map(name => createButton(name, "header-option", () =>
            addTextToTextArea(`[${name}]`)));
			...
			if (!existingheaderLyrics) {
				lyricsEditorContainer.prepend(headerLyrics);
			}
