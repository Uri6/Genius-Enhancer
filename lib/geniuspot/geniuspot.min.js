...
function swapAppleMusicPlayer(clientId, clientSecret) {
    ...
    if (appleMusicParent == undefined) {
        ...
        if (!newSite) {
            ...
            artist = document.getElementsByClassName("Link__StyledLink-h3isu4-0 PcIZE SongHeaderdesktop__Artist-sc-1effuo1-11 fPVhsa")[0].innerText;
        }

        let nameToSearch = [title, artist];

        for (let i = 0; i < nameToSearch.length; i++) {
            if (containsHebrew(nameToSearch[i])) {
                const langsParts = nameToSearch[i].split(" - ")
                nameToSearch[i] = langsParts[1];
            }
        }

        console.log(nameToSearch);

        mapIdAndLoad(clientId, clientSecret, nameToSearch, appleMusicParent, newSite);
